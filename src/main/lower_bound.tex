...
As we have already discussed, for any instance $I$ with
...
\begin{definition}[Opinion Dependent Dynamics]\label{d:opinion_dependent_dynamics}
  Let $A = (A_t)_{t=1}^{\infty}$, where $A_t : [0,1]^{t+2} \mapsto [0,1]$
  be a sequence of functions. $A$ produces the opinion
  dependent dynamics $x_A(t)\in[0,1]^n$ defined as
  \[
    x_{A,i}(t) = A_t(y_i(1),\ldots, y_i(t-1), a_i, s_i),
  \]
  where $y_i(t)$ is the opinion of the neighbor that agent $i$ meets
  at round $t$.
\end{definition}
For convenience we restate our lower bound, Theorem~\ref{t:lower_bound}.
\begin{theorem}
