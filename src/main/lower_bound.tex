...
In this section we investigate whether there exists another
opinion dependent update rule that the agents can select and
ensures a better convergence rate to the equilibrium.
We show the following lower bound for the convergence rate.
We first give a rigorous definition of update rules that only
use the expressed opinions of the neighbors of an agent.
\begin{definition}[Opinion Dependent Dynamics]\label{d:opinion_dependent_dynamics}
 ...
 $A$ produces the opinion
  ...
  \]
  where $y_i(t)$ is the opinion of the neighbor that agent $i$ meets
  at round $t$.
\end{definition}
...
(1-\alpha)\sum_{\tau=0}^{t-1}y_\tau/t + \alpha s$. For convenience we restate our lower bound, 
Theorem~\ref{t:lower_bound}.
\begin{theorem}
...
To simplify notation we will also write it as $E_p[|\theta_t - p|]$.
The risk $E_p[|\theta_t - p|]$ quantifies the error rate of 
the estimated value $\hat{p} =\theta_t(Y_1,\ldots,Y_t)$ to the 
real parameter $p$ as the number of samples $t$ grows. Since $p$ is unknown, any meaningful estimator $\theta=(\theta_t)_{t=1}^\infty$
must guarantee that for all $p \in [0,1]$, $\lim_{t \to \infty} E_p[|\theta_t - p|]=0$.
For example, \emph{sample mean} has error rate
$E_p[|\theta_t-p|] \leq \frac{1}{2\sqrt{t}}$ for any $p \in [0,1]$ and clearly satisfies
the above requirement.

We show now that any opinion dependent algorithm $A$, achieving the
...
is the well-known Cramer-Rao inequality. Let the function $\theta_t: \{0,1\}^t \mapsto [0,1]$ 
such that $E_p[\theta_t]=p$ for all $p \in [0,1]$,  then
...
$\theta =(\theta_t)_{t=1}^\infty$. An estimator $\theta =(\theta_t)_{t=1}^\infty$ is \emph{unbiased}
if $E_p[\theta_t]=p$ for all $p \in [0,1]$. Obviously, we need to prove the claim for any
estimator $\theta$, however this is a first indication that our claim holds.
...
\begin{lemma}\label{l:estimation_lower_bound}
  ...
  random variable with error rate $\Expnew{p}{|\theta_t  - p |}$.
...
\begin{proof}
  ...
  \]
  %
  For any estimator with error $\Expnew{p}{|\theta_t  - p |}$ there exists another estimator
  with $R'_p(t)$ that satisfies the above property and
  $R_p'(t) \leq \Expnew{p}{|\theta_t  - p |}$.
  ...
  \[
    A_k=\{p\in [0,1]: \text{for all }t \geq k,~ t^{1+c}\Expnew{p}{|\theta_t  - p |}\leq 1/2\}
  \]
  %
