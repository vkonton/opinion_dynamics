...
\begin{proof}
  ...
  \begin{figure}
    ...
    \begin{tikzpicture}
      \begin{scope}[every node/.style={circle,draw}]
        ...
        \node (2) at (4,0) [label=below:$a_2 \equal 1/2\comma s_2 \equal 0$]{2};
      \end{scope}
      \begin{scope}[>={Stealth[black]},
        every node/.style={fill=white,circle},
        every edge/.style={draw=black}]
        \path [->] (C) edge[bend right] node {$q$} (1);
        \path [->] (C) edge[bend right] node {$1-q$} (2);
  ...
  Meaning that $\lim_{t \rightarrow \infty}t^{1+c}E_p[|\theta_t^A -p|]=0$ for all
  $p \in [0,1]$.
\end{proof}
%
...
\begin{proof}
  ...
  At first we write the set $B_k$ as
  \[
