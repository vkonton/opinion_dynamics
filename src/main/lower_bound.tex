...
\begin{definition}[Opinion Dependent Dynamics]\label{d:opinion_dependent_dynamics}
 ...
 $A$ produces the opinion
  ...
  at round $t$.
\end{definition}
\noindent For example the update rule~(\ref{eq:fictitious_play}) is \emph{opinion dependent} since
...
results extend trivially them (see Remark~\ref{}). 

We are interested in lower bounds about the convergence rate of
\emph{opinion depedent dynamics} because these bounds also hold
...
For convienience we also write it as $E_p[|\theta_t - p|]$.
...
\begin{lemma}\label{l:reduction}
  Let $A$ an opinion dependent update rule s.t. for all instances $I$,
...
Theorem~\ref{t:estimation_lower_bound}.
...
\begin{theorem}\label{t:estimation_lower_bound}
...
\noindent  Let an estimator $\theta = \{\theta_t\}_{t=1}^{\infty}$, where
  ...
  \]
  %
\noindent  For any estimator $\theta$ with error rate $\Expnew{p}{|\theta_t  - p |}$ there exists another 
  ...
  values.
  %
\noindent  Let $A$ denote the set of $p$ for which the estimator has error
  rate $o(1/t^{1+c})$, that is
  %
  \[
    A= \{p\in [0,1]: \lim_{t \to \infty} t^{1+c}\Expnew{p}{|\theta_t  - p |}=0\}
  \]
 \noindent We show that if we select $p$ uniformly at random in $[0,1]$ then
  $\Prob{p \in A} = 0$.  We also define the set
  \[
    A_k=\{p\in [0,1]: \text{for all }t \geq k,~ t^{1+c}\Expnew{p}{|\theta_t  - p |}\leq 1\}
  \]
  %
\noindent  Observe that if $p \in A$ then there exists $t_p$ such that
  ...
  \]
  %
 \noindent To complete the proof we show that $\Prob{p \in A_k}=0$ for all $k$.
 ...
 \noindent We now show that $A_k \subseteq B_k$.
  ...
  \[
    ...
    1/2.
  \]
\noindent Thus, $\Prob{p \in A_k} \leq \Prob{p \in B_k}$.
...
\end{figure}
 \noindent Each value $\theta_t(i)$ \enquote{covers} length $1/t^{1+c}$ from
