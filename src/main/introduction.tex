...
an network to set their internal opinion equal to 1 so as to maximize the sum
...
consist the FJ model unsuitable for modeling the opinion formation process in
...
environments, we consider that the agents are engaged in the above one-shot
...
We introduce a simple and intuitive update rule that and we show that if the agents
...
process, we deal it with by using elegant concentration arguments.  In
