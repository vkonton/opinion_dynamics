...
$q \in \Q\cap[0,1]$ of Bernoulli random variable
...
of the Appendix.  For a given a $q \in \Q\cap[0,1]$, we construct an instance $I_q$ such that
$x_c^*=q$ for an agent $c$. Moreover, agent $c$ must
receive only values $1$ or $0$ with probability $q$ and $1-q$ respectively.
...
$p \in [0,1]$. At this point we should mention that it is known
...
to use them. In order to explain the various limitations and pitfalls of
these methods and results we will briefly discuss some of them.

 To the best of our knowledge, these sample complexity lower bounds in the
 statistics literature are given as lower bounds for the \emph{minimax risk},
 which, for the case of estimating the mean of a Bernoulli random variable,
 was defined by Wald in \cite{Wal39} as
\[
...
Another option would be to use the well-known Cramer-Rao lower bound
which, if applied for Bernoulli random variables, states that
\[
  \Expnew{p}{(\theta_t - p)^2} \geq \frac{p(1-p)}{t}.
\]
The problem with this bound is that it assumes that the estimator
$\theta_t$ is unbiased, that is $\Exp{p}{\theta_t} = p$. Apart
from that this lower bound works for us, we could set
$p =1/2$ in our instance of \ref{fig:lb_instance} and get a
lower bound for the mean squared error.
The assumption that the estimator is unbiased is considered rather
restrictive and unrealistic even in the statistics literature, and we remark
that our fictitious play dynamics does not correspond to an unbiased estimator.
Thus we would like to get a lower bound with no assumptions about the
estimator.
Our next lemma states something significantly stronger in case $p \in [0,1]$.
For \emph{almost all} $p \in [0,1]$, any estimator $\hat{\theta}$ cannot
achieve this error rate. More precisely,  suppose we select a $p$ uniformly at
...
is possible and we do this without assumptions about the estimator.
