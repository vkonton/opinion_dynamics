...
\begin{figure}\
  ...
  \begin{tikzpicture}
    \begin{scope}[every node/.style={circle,draw}]
      \node (C) at (0,0) [label=below:$a_c \equal 1/2$][label=above:$s_c \equal 0$]{C};
      \node (1) at (-4,0)[label=below:$a_1 \equal 1$][label=above:$s_1 \equal 1$]{1};
      \node (2) at (4,0) [label=below:$a_2 \equal 1$][label=above:$s_2 \equal 0$]{2};
    \end{scope}
