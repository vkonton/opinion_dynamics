...
\begin{definition}\label{d:instance}
  ...
  as $(P,s,\alpha)$, where
    ...
    $\alpha \in (0,1]^n$ the self confidence coefficient vector.
\end{definition}
%
Following the notation introduced earlier we have that
$p_{ij} = w_{ij}/(\sum_{j \in N_i}w_{ij}+w_i)$ if $j \in N_i$ and $0$ otherwise.
...
the Follow the Leader dynamics $x(t)$ is defined in \ref{alg:FTL_dynamics} and
...
\begin{itemize}
 ...
 \item At each round $t\geq 1$, each agent $i$ selects her opinion as:
