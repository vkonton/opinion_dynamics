...
\begin{remark}
  It is necessary that each \enquote{knows} the row $i$ of matrix $P$ for
  this update rule to work. We mention that the lower bound of
  Section~\ref{s:lower_bound} still holds even if the agents also use the indices
  of the neighbors that they meet to update their opinion.
  Theorem~\ref{t:lower_bound} can be easily modified to
  cover this case.
  Observe that Dynamics~\ref{alg:influence_dependent} uses the exact
  values of the weights $w_{ij}$ and this is inevitable for any update
  rule that achieves exponential convergence.
