...
\begin{remark}
  ...
  why \emph{Row Dependent dynamics} converge exponentially
  fast. We mention that the lower bound of
  ...
  ensures exponential convergence must require that each agent 
  $i$ is \emph{aware} of the $i$-th row of matrix $P$.
  
%  requires that 
%  each agent $i$ \enquote{knows} the row $i$ of matrix $P$ for
%  uses the exact
%  values of the weights $p_{ij}$ and this is inevitable for any update
%  rule that achieves exponential convergence rate.
